...
layui.define(['jquery','layer'],function(exports){
  ...
  Class.prototype.init = function(){
    ...
    if(val.length>0){
    	var arr = val.split(options.splitFlag);
    	options.content = arr;
    	options.elem.val('');
    	options.elem.data("val",options.content.join(options.content.splitFlag));
    }
    var isView = options.elem.attr("tagView");
    if(isView=="true"){
    	options.elem.hide();
    }
  ...
  Class.prototype.enter = function(){
    ...
    options.elem.keypress(function(event){  
      ...
      if(keynum == '13'){  
        ...
        if(!$val) return false;
        if(options.content.length+1>options.num){
          	 layer.msg("标签最大个数不超过"+options.num+"个", {icon: 5});
          	 return false;
          }
        if($val.length>options.max){
        	 layer.msg("标签内容长度不超过"+options.max+"位", {icon: 5});
        	 return false;
        }
...
}).link(layui.setter.base+'/css/inputTags.css')