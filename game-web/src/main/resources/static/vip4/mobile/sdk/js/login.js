...
$(function () {
    ...
    //确定注册
    function confirm_register_fn(userName, passWord, superPass, authCode, obj) {
        // 处理按钮
    ...
    //同意协议
        ...
        .on('touchend', '.setPassword .common_btn', function () {
            ...
            if (_this.eq(0).val() != '') {
				...
				if (_this.eq(1).val() !== _this.eq(2).val()) {
                    popup_fn('两次登录密码输入不一致，请重新输入');
                    return;
                }
                if (_this.eq(3).val() === _this.eq(1).val()) {
                ...
                if (_this.eq(5).val().length < 4) {
                    popup_fn('验证码不正确.');
                    return;
                }
                verification_code = _this.eq(1).val();
                //引导实名认证，注册时不强制实名
                confirm_register_fn(_this.eq(0).val(), _this.eq(1).val(), _this.eq(3).val(), _this.eq(5).val(), $('.setPassword'));
            } else {
                popup_fn('请设置登录密码');
            }
        })
...
//			}
        ...
        .on('touchend', '.setNewPassword .common_btn', function () {
            ...
            if (aInput.eq(2).val() !== aInput.eq(3).val()) {
                ...
                // sen_new_password_fn($('.setNewPassword'), mode, bindname, username, newPwd, aInput.eq(0).val())
...
//			}
    ...
    function sen_new_password_fn_new(obj, account, superPass, newPassword) {
        $.ajax({
            type: "get",
            url: login_url + "login/mobile_two!modifyPwdInterFaceNew.action",
            data: {
                "account": account,
                "newPwd": jsObject.getDesPwd(newPassword),
                "superPass": superPass,
            },
            dataType: 'jsonp',
            jsonp: 'jsonpcallback',
